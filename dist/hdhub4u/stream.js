  ...
  return __async(this, arguments, function* ({
    ...
    if (link.includes("hubdrive")) {
      ...
      if (hubdriveLink.includes("hubdrive")) {
        ...
        const hubdriveText = hubdriveRes.data;
        const $$ = cheerio.load(hubdriveText);
        hubdriveLink = $$(".btn.btn-primary.btn-user").attr("href") || hubdriveLink;
      }
    ...
    try {
      ...
      hubcloudLink = ((_d = hubcloudText.match(
        /<META HTTP-EQUIV="refresh" content="0; url=([^"]+)">/i
      )) == null ? void 0 : _d[1]) || hubdriveLink;
    } catch (error) {
      console.log("Error fetching hubdrive link:", error == null ? void 0 : error.message);
    }
...
function getRedirectLinks(link, signal, headers) {
  return __async(this, null, function* () {
    try {
      const res = yield fetch(link, { signal });
