...
function gofileExtractor(id, axios) {
  return __async(this, null, function* () {
    try {
      const gofileRes = yield axios.get("https://gofile.io/d/" + id);
      const genAccountres = yield axios.post("https://api.gofile.io/accounts");
      const token = genAccountres.data.data.token;
      console.log("gofile token", token);
      const wtRes = yield axios.get("https://gofile.io/dist/js/global.js");
...
__name(LALLJLutmoZpvvbikjaWM, "LALLJLutmoZpvvbikjaWM");
function getOrCreateUID() {
  const uid = "uid_" + Date.now() + "_" + Math.random().toString(36).substr(2, 9);
  return uid;
}
__name(getOrCreateUID, "getOrCreateUID");
var getStream = /* @__PURE__ */ __name(function(_0) {
  return __async(this, arguments, function* ({
    ...
    __name(generateMessageToken, "generateMessageToken");
