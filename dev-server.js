...
class DevServer {
  constructor() {
    ...
    // Support both development (desktop-app/web) and release (web) structures
    const devWebDir = path.join(__dirname, "desktop-app", "web");
    const releaseWebDir = path.join(__dirname, "web");
    this.webDir = fs.existsSync(devWebDir) ? devWebDir : releaseWebDir;
  ...
  setupMiddleware() {
    ...
    // Serve static files from web directory (for the portal)
    this.app.use("/web", express.static(this.webDir));
